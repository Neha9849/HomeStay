...
contract("HomeStay", ([landlord, tenant]) => {
    ...
    describe("tests the createRoom function", async () => {
      ...
      it("tests for existence of room name", async() => {
        await homeStay.createRoom("", roomType, imgHashes, description, web3.utils.toWei(rentPerDay.toString(), "ether"), numberOfGuestsCanStay, lat, long).should.be.rejected;
      });
      it("tests for existence of room description", async() => {
        await homeStay.createRoom(roomName, roomType, imgHashes,"", web3.utils.toWei(rentPerDay.toString(), "ether"), numberOfGuestsCanStay, lat, long).should.be.rejected;
      });

      it("checks for the return value", async() => {
        const returnValue = await homeStay.createRoom.call(roomName, roomType, imgHashes, description, web3.utils.toWei(rentPerDay.toString(), "ether"), numberOfGuestsCanStay, lat, long, {from : landlord});
        assert.equal(returnValue, true, "should return true");
      });
      it("checks if room is created", async () => {
        await homeStay.createRoom(roomName, roomType, imgHashes, description, web3.utils.toWei(rentPerDay.toString(), "ether"), numberOfGuestsCanStay, lat, long, {from : landlord});
      ...
      it("checks if the RoomCreated event is emitted with correct values", async () => {
        const result = await homeStay.createRoom(roomName, roomType, imgHashes, description, web3.utils.toWei(rentPerDay.toString(), "ether"), numberOfGuestsCanStay, lat, long, {from : landlord});
        assert.equal(result.receipt.logs.length, 1, "should trigger one event");
        ...
        assert.equal(result.receipt.logs[0].args._owner, landlord, "logs the owner");
      });
    });
    ...
    describe("tests the getAllRooms function", async () => {
      it("return all rooms data", async () => {
        ...
        await homeStay.createRoom(roomName, roomType, imgHashes, description, web3.utils.toWei(rentPerDay.toString(), "ether"), numberOfGuestsCanStay, lat, long, {from : landlord});
        await homeStay.createRoom(roomName1, roomType1, imgHashes1, description1, web3.utils.toWei(rentPerDay1.toString(), "ether"), numberOfGuestsCanStay1, lat1, long1, {from : landlord});
